...
public:
  PhaseShifter(void);
  PhaseShifter(int si_pin, int le_pin, int clk_pin, uint8_t address = 0, mode_e mode = MODE_BINARY_WEIGHTED);
  ...
  mode_e getMode(void);
  void setPins(int si_pin, int le_pin, int clk_pin);
  void getPins(int *si_pin, int *le_pin, int *clk_pin);
};
